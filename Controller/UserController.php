...
class UserController extends Controller
{
    ...
    public function createAction(Request $request)
    {
        ...
        */
        ...
        if ($form->isValid()) {
            ...
            $entity->setPassword($form['thepassword']->getData());
            ...
            $path = $this->getParameter('users_photos');
            $entity->upload($this->getUser()->getId(), $path, $form['fileName']->getData());
            $em->persist($entity);
            $em->flush();
    ...
    */
    private function createEditForm(User $entity)
    {
        ...
        $form->add('thepassword', 'repeated', array('required'=>false, 'mapped'=>false,
                'type' => 'password',
                'first_name' => 'pass',
                'second_name' => 'confirm',
    ...
    public function updateAction(Request $request, $id)
    {
        ...
        */
        ...
        if ($editForm->isValid()) {
            ...
            $newpassword = $editForm['thepassword']->getData();
            if ($newpassword) {
            ...
            $path = $this->getParameter('users_photos');
            $entity->upload($this->getUser()->getId(), $path,$editForm['fileName']->getData());
            $em->flush();
   ...
   /**
    ...
    /**
    * @Route("/loaduserphoto", name="loaduserphoto")
    */
    public function loaduserphotoAction(Request $request) {
        ...
        $uploadDir = $this->getParameter('users_photos') . '/tmp';
    ...
    public function searchuserAction(Request $request) {
        ...
        if (!$pattern2) {
            ...
            $poboxs = $em->getRepository('NvCargaBundle:Pobox')->createQueryBuilder('p')
                ...
                ->getQuery()
                ->getResult();
            $entities = $em->getRepository('NvCargaBundle:User')->createQueryBuilder('o')
                ->where('o.name LIKE :name OR o.lastname LIKE :lastname OR o.pobox IN (:poboxs)' )
                ->andWhere('o.status = :status')
                ->andwhere('o.maincompany = :maincompany')
                ->setParameters(['maincompany'=>$maincompany, 'name'=>$pattern.'%','lastname'=>$pattern.'%', 'poboxs'=>$poboxs,'status'=>$statususer])
                ->setFirstResult(0)
                ->setMaxResults(25)
                ->getQuery()
                ->getResult();
            ...
            $entities = $em->getRepository('NvCargaBundle:User')->createQueryBuilder('o')
                ...
                ->andWhere('o.status = :status')
                ->andWhere('o.maincompany = :maincompany')
                ->setParameters(['maincompany'=>$maincompany,'name'=>$pattern1.'%','lastname'=>$pattern2.'%', 'status'=>$statususer])
                ->setFirstResult(0)
                ->setMaxResults(25)
                ->getQuery()
                ->getResult();
        ...
        foreach ($entities as $user) {
            $result[$counter]['userid'] = $user->getId();
            $result[$counter]['name'] = $user->getName();
            $result[$counter]['lastname'] = $user->getLastname();
            $result[$counter]['email'] = $user->getEmail();
            $result[$counter]['username'] = $user->getUsername();
            if ($user->getPobox()) {
                $result[$counter]['pobox'] = $user->getPobox()->getNumber();
            } else {
                $result[$counter]['pobox'] = 'N/A';
            }
            $counter++;
        }
        return new JsonResponse($result); 
