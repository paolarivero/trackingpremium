   ...
   /**
    ...
    public function createAction(Request $request)
    {
        ...
        */
        ...
        if ($form->isValid()) {
            ...
            $entity->open();
            /*
            // ELIMINADA LA CREACION DEL PRIMER MOVIMIENTO
            ...
            */
            ...
            if ($maincompany->getFirststatus()) {
                ...
                $newstatus = new Statusconsol();
                $newstatus->setConsol($entity);
                $movdate = new \DateTime();
                $clock = $form['clock']->getData(); 
                $rdate = substr($movdate->format('m/d/Y'),0,10) . 'T' . $clock;
                $newstatus->setDate(new \DateTime($rdate));
                $newstatus->setPlace($entity->getAgency()->getCity());
                $newstatus->setStep($step);
                $newstatus->setComment($translator->trans('Consolidado') . ' creado');
    ...
    public function removeAction(Request $request, $id)
    {
        ...
        if ($editForm->isValid()) {
            ...
            $countconsol = $maincompany->getCountconsolidates();
            $maincompany->setCountconsolidates($countconsol--);
            foreach ($entity->getListstatus() as $move) {
                $entity->removeListstatus($move);
                $em->remove($move);
            }
            $em->remove($entity);
            $em->flush();
   ...
   /**
    ...
    public function guidesAction(Request $request)
    {
        ...
        foreach ($guides as $guide) {
            ...
            // MODIFICADO PARA VERIFICAR QUE YA TIENE UN MOVIMIENTO
    ...
    public function remguideAction(Request $request)
    {
        ...
        if ($bag) {
            ...
            foreach ( $listbag as $guide ) {
                ...
                foreach ($guide->getMoves() as $moves) {
                    if (($moves->getStatus()->getIsinherited()) || ($moves->getStatus()->getName() == 'Consolidada')) {
            ...
            foreach ($entity->getMoves() as $moves) {
                if (($moves->getStatus()->getIsinherited()) || ($moves->getStatus()->getName() == 'Consolidada')) {
